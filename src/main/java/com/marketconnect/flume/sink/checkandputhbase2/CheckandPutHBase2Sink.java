package com.marketconnect.flume.sink.checkandputhbase2;
 ...
 */
public class CheckandPutHBase2Sink extends AbstractSink implements Configurable, BatchSizeSupported {

  public static final String CONFIG_CHECK_COLUMN_NAME = "checkColName";
  public static final String CONFIG_CHECK_COLUMN_VALUE = "checkColValue";

  private String tableName;
  private byte[] columnFamily;
  private byte[] checkColName;
  private byte[] checkColValue;
  ...
  public CheckandPutHBase2Sink() {
    this(HBaseConfiguration.create());
  }

  public CheckandPutHBase2Sink(Configuration conf) {
    this.config = conf;
  }

  @VisibleForTesting
  @InterfaceAudience.Private
  CheckandPutHBase2Sink(Configuration conf, DebugIncrementsCallback cb) {
    this(conf);
  ...
  public void configure(Context context) {
    if (!this.hasVersionAtLeast2()) {
      throw new ConfigurationException(
              "HBase major version number must be at least 2 for checkandputhbase2sink");
    }
    ...
    String ccn = context.getString(
        CheckandPutHBase2Sink.CONFIG_CHECK_COLUMN_NAME);
    String ccv = context.getString(
        CheckandPutHBase2Sink.CONFIG_CHECK_COLUMN_VALUE);
    ...
    checkColName = ccn.getBytes(Charsets.UTF_8);
    if (ccv != null && ccv.length() > 0)
        checkColValue = ccv.getBytes(Charsets.UTF_8);
    try {
    ...
    privilegedExecutor.execute((PrivilegedExceptionAction<Void>) () -> {
      for (Row r : actions) {
        if (r instanceof Put) {
            ...
            Table.CheckAndMutateBuilder b = t.checkAndMutate(r.getRow(), columnFamily).qualifier(checkColName);
            if (checkColValue != null) {
                b = b.ifEquals(checkColValue);
            } else {
                b = b.ifNotExists();
            }
            b.thenPut((Put) r);
